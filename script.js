...
// Inisialisasi Supabase (Pakai window.supabase agar aman dari bentrok variable)
const db = window.supabase.createClient(supabaseUrl, supabaseKey);

// ==========================================
// VARIABLE GLOBAL
// ==========================================
let allMembersCache = []; // Tempat simpan data sementara
let diagram = null;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 1. Cek Login (Kecuali di halaman index.html)
...
async function fetchMembersFromSupabase() {
    try {
        ...
        // MAPPING: Ubah kolom Bahasa Indonesia (Database) ke Inggris (Aplikasi)
        allMembersCache = data.map(m => ({
            name: m.Nama,
            uid: String(m.UID),
            upline: m.Upline,
            joinDate: m.TanggalBergabung || new Date().toISOString()
        }));
...
async function addMember() {
    ...
    // Kirim ke Supabase (Pakai Nama Kolom Indonesia)
...
async function saveEditedMember() {
    ...
    if (error) {
        ...
        // Jika UID ganti, update juga anak buahnya (Downline)
...
async function deleteMember(uid) {
    // Putuskan hubungan downline dulu (jadi yatim piatu)
...
// ==========================================
// FUNGSI TAMPILAN & LOGIKA LAINNYA
// ==========================================
...
// Update Jumlah Anggota di Dashboard
...
function renderGrowthChart() {
    ...
    // Tampilkan 12 periode terakhir saja agar rapi
