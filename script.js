...
// ==========================================
// FUNGSI UTAMA: AMBIL DATA (ANTI-GAGAL)
// ==========================================
async function fetchMembersFromSupabase() {
    try {
        const { data, error } = await db.from('members').select('*');
        
        // JIKA ERROR: Tampilkan pesan aslinya biar ketahuan salahnya apa
        if (error) {
            console.error("Supabase Error:", error);
            alert("Error Database: " + error.message + " (Cek RLS di Supabase!)"); 
            throw error;
        }

        // MAPPING PINTAR (Cek semua kemungkinan nama kolom)
        allMembersCache = data.map(m => {
            // Cek variasi nama kolom (Huruf Besar/Kecil/Inggris/Indo)
            ...
            return {
                name: rawName,
                uid: String(rawUid), // Paksa jadi Text
                upline: rawUpline,
                joinDate: rawDate
            };
        });
...
async function addMember() {
    ...
    // Coba kirim dengan format Standar (Kecil)
    const { error } = await db.from('members').insert([{ 
        nama: name, uid: uid, upline: upline || null, tanggalbergabung: joinDate 
    }]);

    // Jika gagal, coba kirim dengan format Kapital (Backup)
    if (error) {
        console.log("Gagal format kecil, coba kapital...", error);
        const { error: error2 } = await db.from('members').insert([{ 
            Nama: name, UID: uid, Upline: upline || null, TanggalBergabung: joinDate 
        }]);
        
        if (error2) {
             showNotification("Gagal simpan: " + error2.message);
...
async function saveEditedMember() {
    ...
    const updates = { 
        // Kita update kolom huruf kecil (standar)
        nama: newName, uid: newUid, upline: newUpline || null, 
    ...
    // Jika gagal, mungkin kolomnya huruf besar? Coba update versi kapital
    if (error) {
        const updatesCap = { 
            Nama: newName, UID: newUid, Upline: newUpline || null, 
            TanggalBergabung: newJoinDate ? new Date(newJoinDate).toISOString() : null
        ...
        if(error2) error = error2; // Jika masih error, berarti memang error beneran
        else error = null; // Jika berhasil, clear error
    }
    ...
    if (error) {
        ...
        if (originalUid !== newUid) {
             // Update downline (Coba 2 versi kolom juga)
...
async function deleteMember(uid) {
    // Putuskan downline (Coba 2 versi kolom)
    await db.from('members').update({ upline: null }).eq('upline', uid);
    await db.from('members').update({ Upline: null }).eq('Upline', uid);
    
    // Hapus data (Coba cari pakai UID kecil dulu, lalu besar)
    let { error } = await db.from('members').delete().eq('uid', uid);
    if(error) {
        const { error: error2 } = await db.from('members').delete().eq('UID', uid);
        error = error2;
    }
...
// ==========================================
// FUNGSI TAMPILAN (TIDAK PERLU DIUBAH)
// ==========================================
function login() {
...
function getDownlineCount(list, parentUid) {
    ...
    for (const child of children) count += getDownlineCount(list, child.uid); 
    return count;
}
...
function getDownlineHierarchyFlat(list, parentUid) {
    ...
    for (const child of children) {
        result.push(child);
        result = result.concat(getDownlineHierarchyFlat(list, child.uid));
    }
    return result;
}
...
function renderGrowthChart() {
    ...
    const finalLabels = labels.slice(-12);
    const finalData = periodData.slice(-12);
...
function ensureFullScreen() {
    ...
    if (!document.fullscreenElement) {
        if (element.requestFullscreen) element.requestFullscreen().catch(err => {});
    }
}
